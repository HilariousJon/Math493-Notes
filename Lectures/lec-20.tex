...
\begin{section}{Composition Series and Jordan-H\"older Theorem}
  ...
  \begin{proof}[\textbf{Sketch of Proof}]
    proof by induction on \( \abs{G} \), and \textbf{glue the composition series} of \( N \) and \( \quotient{G}{N} \) if there exists \( N \unlhd G \).
  \end{proof}
  ...
  \end{remark}
  \begin{proof}[\textbf{Sketch of Proof}]
    This directly yields by doing prime factorization on \( \abs{G} \) and matching the size of the simple factor on the prime number and combined with \textbf{Proposition} \ref{prop:decomp}.
  \end{proof}
